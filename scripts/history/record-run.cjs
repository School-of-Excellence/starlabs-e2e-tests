...
const meta = {
  ...
  // MULTI-SUITE (D1, 2026-07-02): a matrix run shares ONE githubRunId across N suite jobs, so the
  // console queries where('githubRunId'==…) and gets one doc per suite (suite tabs).
  githubRunId: env('GITHUB_RUN_ID', ''),
  // CF source the gate ran against — shown in the console Report screen header.
  cfRepo: env('CF_REPO', ''),
  cfBranch: env('CF_BRANCH', ''),
  createdAt: new Date().toISOString(),
};
// Suite is part of the auto id (D1): matrix suite jobs of the same workflow run must never collide
// on the Firestore doc id or the Storage prefix derived from it.
const runId = (env('RUN_ID') || `${meta.repo}-${meta.branch}-${meta.suite}-${meta.sha.slice(0, 7)}-${Date.now()}`)
...
(async () => {
  ...
  // Direct pointer to the machine-readable per-test report (if the producer emitted one) — the
  // console Report screen fetches THIS instead of walking storage.report[].
  const reportJson = report.find((u) => u.endsWith('/report.json')) || '';

  await docRef.set({
    ...meta,
    runUrl: env('RUN_URL', ''),
    storage: { base: `gs://${BUCKET}/${base}`, report, reportJson, attachments },
  });

...
/**
 * EMULATOR-AWARE admin init, shared by every emulator-gated seeder (authroles/journey/…). When
 * FIRESTORE_EMULATOR_HOST is set (the hermetic emulator run), init an emulator-pinned admin (projectId
 * from FIREBASE_PROJECT, default starlabs-cicd) — firebase-admin then routes Firestore/Auth to the local
 * emulator automatically. Otherwise fall through to the cloud allowlist-guarded seed.initAdmin() (which
 * hard-aborts off the dedicated test project). One copy here instead of one per seeder (handoff: the shared
 * lib that makes ranks 3–10 config-only). Mirrors fixtures/seed-emulator.js's initEmulatorAdmin().
 */
...
module.exports = { TAG, initAdminAuto, seedDashboardRoutes, bootstrapGroup, teardownGroup, seed };

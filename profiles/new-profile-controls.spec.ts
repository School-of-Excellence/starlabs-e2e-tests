...
test.describe('New Profile (/ProfileScreen) — controls addressable', () => {
  ...
  test('renders and the always-present interactive controls are addressable', async ({ page }) => {
    ...
    await expect(page.getByTestId('np-search-input')).toBeVisible({ timeout: 30_000 });
    await expect(page.getByTestId('np-search-clear')).toBeVisible();

    // Subscription-ends date picker + the three month cards.
    await expect(page.getByTestId('np-subs-date')).toBeVisible();
    await expect(page.getByTestId('np-month-last')).toBeVisible();
    await expect(page.getByTestId('np-month-this')).toBeVisible();
    await expect(page.getByTestId('np-month-next')).toBeVisible();

    // Engagement category radio group + its two options.
    await expect(page.getByTestId('np-engagement-group')).toBeVisible();
    await expect(page.getByTestId('np-eng-active')).toBeVisible();
    await expect(page.getByTestId('np-eng-nonactive')).toBeVisible();

    // The five engagement-level cards + their timeline (menu) triggers.
    await expect(page.getByTestId('np-englevel-disappear')).toBeVisible();
    await expect(page.getByTestId('np-englevel-disappear-timeline')).toBeVisible();
    await expect(page.getByTestId('np-englevel-minimal')).toBeVisible();
    await expect(page.getByTestId('np-englevel-minimal-timeline')).toBeVisible();
    await expect(page.getByTestId('np-englevel-average')).toBeVisible();
    await expect(page.getByTestId('np-englevel-average-timeline')).toBeVisible();
    await expect(page.getByTestId('np-englevel-optimal')).toBeVisible();
    await expect(page.getByTestId('np-englevel-optimal-timeline')).toBeVisible();
    await expect(page.getByTestId('np-englevel-superoptimal')).toBeVisible();
    await expect(page.getByTestId('np-englevel-superoptimal-timeline')).toBeVisible();
  });

  test('engagement-level timeline menu items are addressable (opened per menu)', async ({ page }) => {
    await page.goto('/ProfileScreen', { waitUntil: 'domcontentloaded' });
    await expect(page.getByTestId('np-englevel-disappear-timeline')).toBeVisible({ timeout: 30_000 });

    // Disappear timeline menu — open then assert both items.
    await page.getByTestId('np-englevel-disappear-timeline').click();
    ...
    await page.getByTestId('np-englevel-minimal-timeline').click();
    ...
    await page.getByTestId('np-englevel-average-timeline').click();
    ...
    await page.getByTestId('np-englevel-optimal-timeline').click();
    ...
    await page.getByTestId('np-englevel-superoptimal-timeline').click();
